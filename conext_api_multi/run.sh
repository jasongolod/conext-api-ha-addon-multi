...
# Check permissions on /data/options.json
ls -l /data/options.json > /app/permissions.log 2>&1 || echo "No /data/options.json found"
cat /app/permissions.log
# Copy options.json directly
if [ -f /data/options.json ]; then
    cp /data/options.json /app/raw_config.json 2>/app/config_error.log || {
        echo "Error copying /data/options.json, see /app/config_error.log"
    ...
    echo "No /data/options.json found; using hardcoded fallback config"
    echo '[{"name": "Insight_Facility_1", "ip": "192.168.10.106", "port": 503, "timeout": 5, "batteries": [], "inverters": [], "charge_controllers": []}, {"name": "Insight_Facility_2", "ip": "192.168.10.107", "port": 503, "timeout": 5, "batteries": [], "inverters": [], "charge_controllers": []}]' > /app/config.json
fi
